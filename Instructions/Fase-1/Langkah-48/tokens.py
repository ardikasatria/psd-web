...
def new_opaque(nbytes: int = 32) -> str:
    """Token acak URL-safe, high-entropy (untuk access/refresh/authorization code)."""
...
def make_id_token(
    *,
    ...
    """Bangun ID token OIDC (JWT) yang ditandatangani RS256."""
    now = int(time.time())
    ...
    if "profile" in scopes:
        for claim in ("name", "preferred_username", "picture"):
