...
def test_psi_shifted_is_significant():
    random.seed(2)
    ref = [random.gauss(0, 1) for _ in range(2000)]
    cur = [random.gauss(3, 1) for _ in range(2000)]   # geser mean
    psi = drift.psi_numeric(ref, cur)
    assert psi > 0.25
...
def test_drift_row_shape_and_round():
    r = monitoring.drift_row(model_name="m", model_version="3", feature="x",
                             metric="psi", value=0.123456789, status="moderate")
...
def test_collect_alerts():
    rows = [
        monitoring.drift_row(model_name="m", model_version="1", feature="x",
                             metric="psi", value=0.4, status="significant"),
        monitoring.drift_row(model_name="m", model_version="1", feature="y",
                             metric="psi", value=0.05, status="stable"),
    ]
    alerts = monitoring.collect_alerts(rows)
    assert len(alerts) == 1 and alerts[0]["feature"] == "x"


# -------------------- registry (fake MLflow) --------------------
class FakeMV:
...
def test_registry_register_and_promote():
    ...
    version = svc.register_from_run(name="psd/iris-clf", run_id="run123",
                                    tags={"sumber": "kompetisi"})
    assert version == "4"
    ...
    # source mengarah ke artefak run
...
def test_registry_idempotent_when_model_exists():
    ...
    fake = Existing()
    svc = RegistryService(fake)
    v = svc.register_from_run(name="psd/iris", run_id="r1")   # tidak meledak
    assert v == "4"


# -------------------- drift job end-to-end --------------------
def test_compute_drift_job(monkeypatch):
    random.seed(3)
    ref_num = [random.gauss(0, 1) for _ in range(1000)]
    cur_num = [random.gauss(3, 1) for _ in range(1000)]   # drift signifikan
    ...
    result = drift_job.compute_drift(
        "psd/iris", "4",
        features=[{"name": "umur", "kind": "numeric"},
                  {"name": "wilayah", "kind": "categorical"}],
        write=lambda rows: written.extend(rows),
        alert=lambda a: alerts.append(a),
    )
    # numeric → psi + ks (2 baris); categorical → psi (1 baris) = 3 baris
    ...
    # kedua fitur drift signifikan → 2 alert
