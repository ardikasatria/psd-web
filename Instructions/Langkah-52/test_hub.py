...
Uji logika inti Langkah 52 — tanpa JupyterHub/Docker hidup.
  ...
  - tiers: resolusi tier + fallback + CPU-only (gpu==0).
  - spawn: auth_state_hook + apply_tier_limits set batas & env PSD ke spawner tiruan.
  - psd_sdk: parse_uri, Resolver.resolve (bentuk request), download (isi berkas).
"""
import os
...
def test_resolve_known_and_default():
    ...
    assert tiers.resolve_limits("LANJUT").mem_gb == 8        # case-insensitive
...
async def test_apply_tier_limits_sets_limits_and_env(monkeypatch):
    ...
    await spawn.auth_state_hook(spawner, {
        "access_token": "AT-123",
    ...
    assert not hasattr(spawner, "gpu")          # tak pernah minta GPU
    assert lim.gpu == 0
...
async def test_apply_tier_limits_defaults_when_no_tier(monkeypatch):
    ...
    assert spawner.mem_limit == "2G"            # pemula
...
def test_parse_uri():
    ...
    with pytest.raises(ValueError):
        psd_sdk.parse_uri("psd://budi/iris")     # kurang path
...
def test_download_writes_content(tmp_path):
    ...
    def handler(req: httpx.Request):
        if req.url.path.endswith("/resolve"):
            return httpx.Response(200, json={"presigned_url": "http://minio.local/iris.csv?sig=1"})
        # presigned URL → kembalikan isi CSV
