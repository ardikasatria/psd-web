"""
Submission kompetisi: alur penilaian admin (humas) + leaderboard.

Status: submitted → under_review → scored | rejected (reopen → under_review).
Penilaian dilakukan admin/humas (skor). Leaderboard memakai submission terbaik per
peserta (tim atau solo), arah skor bisa maksimal (akurasi) atau minimal (RMSE).
"""
...
_RESULT = {"start_review": UNDER_REVIEW, "score": SCORED, "reject": REJECTED,
           "reopen": UNDER_REVIEW}
...
def _sort_key(s: dict, higher_is_better: bool):
    sc = s["score"]
    return (-sc if higher_is_better else sc, s["submitted_at"])   # makin kecil = makin baik
