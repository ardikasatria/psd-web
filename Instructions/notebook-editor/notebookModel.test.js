...
test("deleteCell menghapus; tak pernah kosong", () => {
  ...
  assert.equal(nb.cells.length, 1);
  // hapus sel terakhir → tetap ada satu sel kosong
...
test("moveCell naik/turun & batas", () => {
  ...
  nb = m.moveCell(nb, first, "down"); // sudah di bawah → tak berubah
  assert.equal(nb.cells[1].id, first);
});
...
test("ipynb round-trip mempertahankan sumber & tipe", () => {
  ...
  // sel markdown tak boleh punya execution_count/outputs
  const md = ip.cells.find((c) => c.cell_type === "markdown");
