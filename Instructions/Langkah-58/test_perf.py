"""
Uji cache & performa (Langkah 58) — murni Python.
"""
...
def test_get_or_set_computes_once_then_hits():
    ...
    assert out2 == {"v": 42} and from_cache2 is True      # nilai cache, bukan 99
    assert len(calls) == 1                                # compute sekali
...
def test_cached_widget_and_invalidate_run():
    ...
    targets.cached_widget(cache, "run1", "w1", compute)   # hit
    assert len(calls) == 2                                # w1, w2 sekali masing-masing

    removed = targets.invalidate_run(cache, "run1")
    assert removed == 2
    targets.cached_widget(cache, "run1", "w1", compute)   # recompute setelah invalidasi
    assert len(calls) == 3
