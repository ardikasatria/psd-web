...
from . import drift, monitoring, seams


def compute_drift(model_name: str, model_version: str, features: list[dict], *,
                  write=None, alert=None) -> dict:
    ...
    for feat in features:
        ...
        if kind == "categorical":
            ...
            rows.append(monitoring.drift_row(
                model_name=model_name, model_version=model_version, feature=name,
                metric="psi", value=psi, status=drift.classify_psi(psi)))
        else:
