...
def drift_row(*, model_name: str, model_version: str, feature: str,
              metric: str, value: float, status: str,
...
def metric_row(*, model_name: str, model_version: str, metric: str,
               value: float, status: str = "ok",
               computed_at: float | None = None) -> dict:
    """Metrik level-model (akurasi, distribusi prediksi, dll.) — feature='__model__'."""
    return drift_row(model_name=model_name, model_version=model_version,
                     feature="__model__", metric=metric, value=value,
                     status=status, computed_at=computed_at)
