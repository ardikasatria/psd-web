"""
Counter engagement (suka/bagikan/unduh/lihat) per aset + ringkasan agregat per
pengguna sebagai SUMBER TUNGGAL untuk profil (gaya statistik Kaggle / total suka TikTok).

Murni (tanpa DB). Aset bersifat polimorфик: dirujuk lewat kunci "<kind>:<slug>".
Ringkasan pengguna = jumlah counter di seluruh aset miliknya (dijaga inkремental;
recompute_user_summary tersedia untuk rekonsiliasi).
"""
from __future__ import annotations

from dataclasses import dataclass, field

# Saluran berbagi. Unduhan dihitung terpisah (pemakaian, bukan amplifikasi).
SHARE_CHANNELS = {"feed", "forum", "external", "link"}
...
def apply_love(asset: AssetCounters, summary: UserSummary, *, loved: bool) -> None:
    """Toggle suka. loved=True menambah, False mengurangi (tak pernah < 0)."""
    d = 1 if loved else -1
...
def recompute_user_summary(owned_assets: list[AssetCounters]) -> UserSummary:
    """Hitung ulang ringkasan dari seluruh aset milik pengguna (rekonsiliasi)."""
