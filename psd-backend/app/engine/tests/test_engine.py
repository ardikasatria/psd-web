"""Uji jalur eksekusi (Langkah 54) — tanpa Spark/Airflow hidup."""
import pytest
...
def sample_pipeline(engine="auto"):
    return Pipeline(
        ...
        nodes=[
            ...
            Node(id="j", type="join", params={"on": "a.k = b.k", "how": "inner"}, inputs=["a", "b"]),
            Node(
                id="agg",
                type="sql",
                params={"sql": "SELECT k, count(*) c FROM j GROUP BY k"},
                inputs=["j"],
            ),
            Node(
                id="out",
                type="sink",
                params={"target": "gold.hasil", "format": "parquet"},
                inputs=["agg"],
            ),
        ],
    )
...
def test_selector_auto_threshold():
    ...
    assert selector.choose_engine(sample_pipeline("auto")) == "duckdb"
...
class FakeDF:
    ...
    def createOrReplaceTempView(self, v):
        self.log.append(("view", v))
...
class FakeSession:
    ...
    def sql(self, q):
        ...
        return FakeDF(self.log, "sql")

    def table(self, t):
        self.log.append(("table", t))
        return FakeDF(self.log, t)
