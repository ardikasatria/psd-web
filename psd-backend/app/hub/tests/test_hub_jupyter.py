"""Uji integrasi JupyterHub (Langkah 52 revisi)."""
import json
...
def make_client(handler):
    return JupyterHubClient("https://hub.psd.example/hub/api", "svc-token", transport=httpx.MockTransport(handler))
...
def test_ensure_server_polls_until_ready():
    ...
    c = make_client(handler)
    model = c.ensure_server("budi", timeout_s=10, interval=0, sleep=lambda s: None, clock=lambda: 0.0)
...
def test_ensure_server_timeout():
    ...
    with pytest.raises(HubError) as e:
        c.ensure_server("budi", timeout_s=5, interval=1, sleep=lambda s: None, clock=lambda: t.__setitem__(0, t[0] + 3) or t[0])
...
class FakeHub:
    ...
    def ensure_server(self, name, server_name=""):
...
def test_open_runtime_returns_config_when_allowed():
    hub = FakeHub()
    cfg = launch.open_server_runtime(
        hub,
        name="budi",
        hub_public_url="https://hub.psd.example",
        grant_active=True,
        running_count=0,
        max_concurrent=2,
    )
