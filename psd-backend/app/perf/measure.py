"""Pengukuran latensi — ukur dulu, baru cache (Langkah 58)."""
...
class Registry:
    ...
    def all_stats(self) -> dict[str, dict]:
        return {name: self.stats(name) for name in self._samples if self.stats(name)}

    def is_slow(self, name: str, threshold_ms: float, *, pct: str = "p95") -> bool:
        s = self.stats(name)
        return bool(s) and s[pct] >= threshold_ms

    def should_cache(self, name: str, *, threshold_ms: float = 200.0, min_samples: int = 30, pct: str = "p95") -> bool:
        s = self.stats(name)
