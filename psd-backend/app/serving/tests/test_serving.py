...
def test_service_measures_latency_and_logs():
    ...
    logs = []
    svc = InferenceService(loader, clock=lambda: next(ticks), log_fn=lambda **kw: logs.append(kw))
...
def test_desired_replicas_bounds():
    ...
    assert scaling.desired_replicas(rps=1000, target_rps_per_replica=10, max_replicas=5) == 5


def test_should_retrain_consecutive():
    assert scaling.should_retrain(["stable", "significant", "significant"]) is False
    assert scaling.should_retrain(["significant", "significant", "significant"]) is True
    assert scaling.should_retrain(["significant", "stable", "significant", "significant"], consecutive=2) is True


def test_latency_logger_writes_row():
    rows = []
    log_fn = make_latency_logger(lambda r: rows.extend(r), version_lookup=lambda n, s: "4")
...
async def test_predict_endpoint_quota_and_success(monkeypatch):
    ...
    from app.serving.deps import get_inference_service, get_quota_store
    ...
    monkeypatch.setattr(ep, "resolve_mlflow_name", _resolve)
    ...
    monkeypatch.setattr(ep.settings, "PSD_SERVING_ENABLED", True)
    monkeypatch.setattr(ep.settings, "PSD_MLFLOW_ENABLED", True)
