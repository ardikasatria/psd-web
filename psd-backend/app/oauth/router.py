"""Endpoint OAuth2/OIDC PSD (Langkah 48)."""
...
from app.oauth.models import OAuthAuthorizationCode, OAuthClient, OAuthConsent, OAuthToken
from app.oauth.scopes import SCOPES, validate_scope
from app.oauth.settings import settings
from app.oauth.tokens import make_id_token, new_opaque
...
def _consent_page(client: OAuthClient, scope: str, request: Request) -> HTMLResponse:
    ...
    items = "".join(f"<li>{SCOPES.get(s, s)}</li>" for s in scope.split())
    hidden = "".join(
...
async def _issue_code(db, client, user, redirect_uri, scope, nonce, cc, ccm) -> str:
    ...
    db.add(
        OAuthAuthorizationCode(
            ...
            nonce=nonce,
            code_challenge=cc,
            code_challenge_method=(ccm if cc else None),
            auth_time=now,
...
async def _issue_tokens(db, client, user_id, scope, nonce, auth_time, claims) -> dict:
    ...
    if "openid" in scopes:
        body["id_token"] = make_id_token(
            ...
            client_id=client.client_id,
            user_claims=_claims_dict(claims),
            nonce=nonce,
            auth_time=auth_time,
            scope=scope,
            expires_in=settings.ID_TOKEN_TTL,
        )
    return body
...
async def authorize(
    ...
    if not redirect_uri or not client.check_redirect_uri(redirect_uri):
        return _error_page("invalid_redirect_uri", "redirect_uri tidak terdaftar untuk klien ini.")
    ...
    code = await _issue_code(
        db, client, user, redirect_uri, scope, nonce, code_challenge, code_challenge_method
    )
    params = {"code": code}
    if state:
        params["state"] = state
...
async def token(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return _token_error(
        ...
        "gunakan authorization_code atau refresh_token.",
        400,
    )
...
async def _grant_auth_code(form, client, db):
    ...
    row = (
        await db.execute(select(OAuthAuthorizationCode).where(OAuthAuthorizationCode.code == code))
    ).scalar_one_or_none()
    ...
    body = await _issue_tokens(
        db, client, row.user_id, row.scope, nonce=row.nonce, auth_time=row.auth_time, claims=claims
    )
    await db.commit()
...
async def _grant_refresh(form, client, db):
    ...
    row = (
        await db.execute(select(OAuthToken).where(OAuthToken.refresh_token == rt))
    ).scalar_one_or_none()
    ...
    if claims is None:
        return _token_error("invalid_grant", "pengguna tidak ditemukan.")
    body = await _issue_tokens(
        db, client, row.user_id, scope, nonce=None, auth_time=int(time.time()), claims=claims
    )
    await db.commit()
    return JSONResponse(body, headers={"Cache-Control": "no-store", "Pragma": "no-cache"})
...
async def userinfo(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    row = (
        await db.execute(select(OAuthToken).where(OAuthToken.access_token == at))
    ).scalar_one_or_none()
