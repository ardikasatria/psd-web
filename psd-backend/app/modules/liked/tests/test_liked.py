...
from app.modules.liked import service, visibility
from app.modules.liked.service import LikedError
from app.modules.liked.visibility import LikedItem, LikedSettings
...
def test_is_asset_key_excludes_feed_forum():
    ...
    assert visibility.is_asset_key("post:123") is False
    assert visibility.is_asset_key("thread:9") is False
...
def test_master_switch_hides_everything_for_others():
    ...
    pub = LikedItem("dataset:a", is_public=True)
    assert visibility.visible_to("x", "u", s, pub) is False
    assert visibility.visible_to("u", "u", s, pub) is True


def test_filter_and_public_count():
    s = LikedSettings(list_public=True)
    items = [
        LikedItem("dataset:a", True),
        LikedItem("dataset:b", False),
...
class FakeStore:
    ...
    def get_item(self, uid, key):
        return self._items.get(key)

    def set_item_public(self, uid, key, pub):
        if key in self._items:
            self._items[key].is_public = pub
        else:
            self._items[key] = LikedItem(key, pub)
...
def test_set_item_visibility_owner_only_and_asset_only():
    ...
    r = service.set_item_visibility(
        store, user_id="u", actor_id="u", asset_key="dataset:a", is_public=False
    )
    assert r["is_public"] is False
    assert store.get_item("u", "dataset:a").is_public is False
    with pytest.raises(LikedError) as e1:
        service.set_item_visibility(
            store, user_id="u", actor_id="x", asset_key="dataset:a", is_public=True
        )
    assert e1.value.status == 403
    with pytest.raises(LikedError) as e2:
        service.set_item_visibility(
            store, user_id="u", actor_id="u", asset_key="post:1", is_public=True
        )
    assert e2.value.status == 422
    with pytest.raises(LikedError) as e3:
        service.set_item_visibility(
            store, user_id="u", actor_id="u", asset_key="dataset:zzz", is_public=True
        )
