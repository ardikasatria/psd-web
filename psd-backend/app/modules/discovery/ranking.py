"""Logika perangkingan panel penemuan komunitas."""
...
def popularity_score(u: dict) -> int:
    return (u.get("follower_count", 0) * W_FOLLOWERS) + (u.get("post_like_total", 0) * W_POST_LIKES)
...
def new_members(
    ...
    *,
    ...
    ex = set(exclude_ids)
    pool = [u for u in users if u.get("id") not in ex and _as_dt(u.get("created_at")) >= cutoff]
