"""Uji AI Asisten & Rekomendasi (Langkah 57)."""
import pytest

from app.assistant import feed as feed_mod
from app.assistant import recommend
from app.assistant.affinity import build_affinity, from_psd_activity_summary
...
def test_from_psd_activity_summary():
    raw = {
        "top_categories": [{"slug": "nlp", "name": "NLP", "count": 3}],
        "top_tags": [{"tag": "teks", "count": 4}],
        "actions": {"view": 8},
    }
    out = from_psd_activity_summary(raw)
    assert out["categories"]["nlp"] == 3
    assert out["event_count"] == 8


def test_build_affinity_normalizes():
    p = build_affinity({"categories": {"nlp": 3, "cv": 1}, "tags": {"teks": 4}, "event_count": 8})
...
def test_affinity_ranks_relevant_first():
    p = build_affinity({"categories": {"nlp": 10}, "tags": {"teks": 10}, "event_count": 20})
    out = recommend.recommend(p, CANDIDATES, popularity=POP, k=4)
...
def test_exclude_and_category_cap():
    p = build_affinity({"categories": {"nlp": 10}, "tags": {"teks": 10}, "event_count": 20})
    out = recommend.recommend(p, CANDIDATES, popularity=POP, k=10, exclude_ids=["d1"], per_category_cap=1)
...
def test_ai_quota_gating():
    ...
    a = AIAssistant(lambda msgs: "halo", store)
    for _ in range(20):
...
async def test_ask_endpoint_quota(monkeypatch):
    ...
    from app.assistant.deps import get_assistant
    ...
    app.dependency_overrides[get_current_user] = lambda: type("U", (), {"id": "u1", "reputation": 0})()
    app.dependency_overrides[get_assistant] = lambda: assistant

    monkeypatch.setattr(ep.settings, "PSD_ASSISTANT_ENABLED", True)
    monkeypatch.setattr(ep.settings, "OPENAI_API_KEY", "test-key")
...
async def test_feed_endpoint_cold_start(monkeypatch):
    ...
    async def _summary(db, user):
        return {"categories": {}, "tags": {}, "event_count": 0}

    async def _candidates(db, kind):
        return CANDIDATES

    async def _popularity(db, kind):
        return POP

    async def _viewed(db, user, days=14):
        return set()

    async def _state(db, user):
        return {"has_published_dataset": False}

    monkeypatch.setattr(ep, "fetch_activity_summary", _summary)
    monkeypatch.setattr(ep, "fetch_candidates", _candidates)
    monkeypatch.setattr(ep, "fetch_popularity", _popularity)
    monkeypatch.setattr(ep, "fetch_viewed_ids", _viewed)
    monkeypatch.setattr(ep, "fetch_user_state", _state)
    monkeypatch.setattr(ep.settings, "PSD_ASSISTANT_ENABLED", True)
